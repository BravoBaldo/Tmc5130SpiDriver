...
#endif
	ID_BTN_TESTS,
	ID_BTN_TESTS_LAST= ID_BTN_TESTS + NUMOFTESTS,
};
...
//	EVT_RIGHT_DOWN			(			OnMouseEvent )
	...
	EVT_BUTTON			(-1, OnBtnCommands)
	//EVT_BUTTON_RANGE	(ID_BTN_Stop, OnBtnCommands)
...
void MyFrame::SetLayouts ( void ) {
	...
	for (int i = 0; i < WXSIZEOF(m_btn_Tests); i++) {
		wxString PanName = wxString::Format("Test %02d", i);
		m_mgr.AddPane(m_btn_Tests[i], wxAuiPaneInfo().Name(PanName).Caption(PanName).Right());
	}
